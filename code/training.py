...
def train_from_video(vid):
    ...
    np.save(r"Dataset\videos\scene1\megaBlockMotInfVal_set1_p1_train_40-40_k5.npy",megaBlockMotInfVal)
    print(np.amax(megaBlockMotInfVal))
    print(np.amax(reject_outliers(megaBlockMotInfVal)))
    
    codewords = cmb.kmeans(megaBlockMotInfVal)
    np.save(r"Dataset\videos\scene1\codewords_set1_p1_train_40-40_k5.npy",codewords)
    print (codewords)
    return
